    ...
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div class="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-xl shadow-2xl relative my-8">
        ...
        <div class="flex justify-between items-center p-6 border-b border-gray-100 dark:border-gray-700">
          ...
          <button @click="closeEditModal" class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 w-10 h-10 rounded-full flex items-center justify-center transition-colors">
            <i class="pi pi-times text-xl"></i>
          </button>
        </div>

        <!-- Modal Body -->
        <div class="p-6 space-y-6" :key="modalKey">
          ...
          <!-- Banner Upload -->
          <div>
            <label class="block text-base font-bold text-gray-700 dark:text-gray-300 mb-2">รูปแบนเนอร์ (Banner)</label>
            <div class="mb-3 rounded-xl overflow-hidden h-32 bg-gray-100 dark:bg-gray-700 relative border border-gray-200 dark:border-gray-600">
               <img v-if="bannerPreviewUrl" :src="bannerPreviewUrl" class="w-full h-full object-cover" />
               <div v-else class="absolute inset-0 flex items-center justify-center text-gray-400">ไม่มีรูปแบนเนอร์</div>
            </div>
            <div class="flex items-center gap-3">
              <input type="file" accept="image/*" @change="onBannerChange" class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2.5 file:px-5 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-blue-900/30 dark:file:text-blue-400 cursor-pointer" />
            </div>
            <p v-if="bannerFile" class="text-sm font-semibold text-green-600 mt-2">เลือกไฟล์: {{ bannerFile.name }}</p>
          </div>

          <!-- Avatar Upload -->
          <div>
            <label class="block text-base font-bold text-gray-700 dark:text-gray-300 mb-2">รูปโปรไฟล์ (Avatar)</label>
            <div class="mb-3 flex items-center gap-4">
              <div class="w-20 h-20 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 flex-shrink-0">
                 <img v-if="avatarPreviewUrl" :src="avatarPreviewUrl" class="w-full h-full object-cover" />
                 <div v-else class="w-full h-full flex items-center justify-center text-gray-400 font-bold text-2xl">
                    {{ (editForm.nickname || profile?.nickname || '?').charAt(0).toUpperCase() }}
                 </div>
              </div>
              <input type="file" accept="image/*" @change="onAvatarChange" class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2.5 file:px-5 file:rounded-xl file:border-0 file:text-sm file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-blue-900/30 dark:file:text-blue-400 cursor-pointer" />
            </div>
            <p v-if="avatarFile" class="text-sm font-semibold text-green-600 mt-2">เลือกไฟล์: {{ avatarFile.name }}</p>
          </div>
        ...
        <div class="p-6 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-4 bg-gray-50/50 dark:bg-gray-800/50 rounded-b-3xl">
          <button @click="closeEditModal" :disabled="isSaving" class="px-6 py-3 rounded-xl text-gray-600 dark:text-gray-300 font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors disabled:opacity-50">
            ยกเลิก
          </button>
          <button @click="openPasswordModal" :disabled="isSaving" class="flex items-center gap-2 px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <div v-if="isPasswordModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div class="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md shadow-2xl relative my-8">
        ...
        <div class="flex justify-between items-center p-6 border-b border-gray-100 dark:border-gray-700">
          ...
          <button @click="closePasswordModal" class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 w-10 h-10 rounded-full flex items-center justify-center transition-colors">
            <i class="pi pi-times text-xl"></i>
          </button>
        </div>
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="bg-red-50 dark:bg-red-900/10 p-5 rounded-2xl border border-red-100 dark:border-red-800/30">
            <label class="block text-base font-extrabold text-red-700 dark:text-red-400 mb-2">
              <i class="pi pi-lock mr-1"></i> ยืนยันรหัสผ่านปัจจุบัน (จำเป็น)
            </label>
        ...
        <!-- Modal Footer -->
        <div class="p-6 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-4 bg-gray-50/50 dark:bg-gray-800/50 rounded-b-3xl">
          <button @click="closePasswordModal" :disabled="isSaving" class="px-6 py-3 rounded-xl text-gray-600 dark:text-gray-300 font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors disabled:opacity-50">
            ยกเลิก
          </button>
          <button @click="saveProfile" :disabled="isSaving || !editForm.currentPassword" class="flex items-center gap-2 px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
