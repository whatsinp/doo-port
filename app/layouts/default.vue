<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
    <!-- Navbar -->
    <nav class="sticky top-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md shadow-sm border-b border-gray-200 dark:border-gray-700">
      <div class="max-w-[1600px] mx-auto px-5 sm:px-8 lg:px-10">
        <div class="flex justify-between h-16">
          <div class="flex items-center">
            <!-- Mobile menu button -->
            <button
class="sm:hidden mr-3 p-1 rounded-md text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:hover:text-white focus:outline-none flex flex-col items-center justify-center"
...
href="/favorites"
          ...
          <div class="flex items-center space-x-4">
            ...
            <div class="relative cursor-pointer mr-2">
              <i class="pi pi-bell text-gray-500 dark:text-gray-300 text-xl hover:text-gray-700"/>
              <span
v-if="unreadCount > 0"
...
class="flex items-center focus:outline-none relative group"
                ...
                <div class="relative">
                  <img
v-if="profile?.avatarUrl" :src="profile.avatarUrl" alt="Profile"
...
v-else
                  ...
                  <div
                    ...
                    <i class="pi pi-chevron-down text-[10px] text-gray-800 dark:text-gray-200 font-bold"/>
                  </div>
                </div>
              </button>
...
v-if="isDropdownOpen"
                  ...
                  <div
                    ...
                    <div class="flex items-center gap-3 mb-4">
                      <img
v-if="profile?.avatarUrl" :src="profile.avatarUrl"
...
v-else
                      ...
                      <span class="text-[17px] font-bold text-gray-900 dark:text-white line-clamp-1">
                        {{ profile?.nickname || auth.user.value?.email }}
                      </span>
                    </div>
                    <NuxtLink
to="/profile" class="block w-full text-center px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-blue-600 dark:text-blue-400 font-bold rounded-lg transition-colors text-sm"
                      @click="isDropdownOpen = false">
                      <i class="pi pi-user mr-1"/> ดูโปรไฟล์ทั้งหมด
                    </NuxtLink>
                  </div>
                  ...
                  <div class="flex flex-col gap-1">
                    <NuxtLink
to="/settings" class="flex items-center justify-between p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group cursor-pointer"
                      @click="isDropdownOpen = false">
                      <div class="flex items-center gap-3">
                        <div
                          ...
                          <i class="pi pi-cog text-xl text-gray-900 dark:text-white"/>
                        </div>
                        <span class="font-semibold text-gray-900 dark:text-white text-[15px]">การตั้งค่า</span>
                      </div>
                      <i
...
class="flex items-center justify-between p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group cursor-pointer w-full text-left"
                      ...
                      <div class="flex items-center gap-3">
                        <div
                          class="w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-600 group-hover:bg-gray-300 dark:group-hover:bg-gray-500 flex items-center justify-center transition-colors shadow-sm">
                          <i class="pi pi-sign-out text-xl text-gray-900 dark:text-white"/>
                        </div>
                        <span class="font-semibold text-gray-900 dark:text-white text-[15px]">ออกจากระบบ</span>
                      </div>
                      <i
                        class="pi pi-angle-right text-gray-500 dark:text-gray-400 text-xl group-hover:text-gray-700 dark:group-hover:text-gray-200"/>
                    </button>
    ...
    <div v-if="isMobileMenuOpen" class="fixed inset-0 z-50 sm:hidden">
      <!-- Overlay -->
      <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" @click="isMobileMenuOpen = false"/>

      <!-- Drawer -->
      <div
        class="fixed inset-y-0 left-0 w-64 bg-white dark:bg-gray-800 shadow-xl overflow-y-auto transform transition-transform duration-300">
        <div class="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-700">
          <span class="text-xl font-bold text-blue-600 dark:text-blue-400">Doo-Port</span>
          <button
class="text-gray-400 hover:text-gray-600 dark:text-gray-400 dark:hover:text-white focus:outline-none p-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 rounded-full transition-colors"
